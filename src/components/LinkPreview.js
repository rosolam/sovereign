...
const LinkPreview = ({attachment}) => {
    ...
    if(attachment.title){
        ...
        <div className='bg-white rounded-corners p-1 w-100'>
            ...
            <a href={attachment.url} target='_new' className='m0 stretched-link'><small className='text-muted ml-2 ellipsis' style={{maxWidth:'250px'}}>{attachment.url}</small></a>
        </div>
        )
    } else return (
        <div className='bg-white rounded-corners p-1'>
            <div className='m-2'><a href={attachment.url} target='_new' className='ellipsis stretched-link' style={{maxWidth:'250px'}}>{attachment.url}</a></div>
        </div>
    )
